...
① 环境诊断 —— 自研:工具发现/校验/恢复
↓
② 准备工作区 —— 自研:输入验证/APK 复制
↓
③ 签名库 —— 第三方:keytool / 自研:密码安全
↓
④ 源码安全审计 —— 自研:project-audit / r8-rules
↓
⑤ 构建 Release APK —— 第三方:Gradle Wrapper
↓
⑥ APK 安全审计 —— 自研:ZIP/DEX/ELF/ARSC/AXML / 第三方:aapt
↓
⑦ 本地安全防护 —— 自研:证据核验/置信度
↓
⑧ Release 归一化 —— 自研:Manifest 改写/调试清理
↓
⑨ Web JS 处理(可选) —— 第三方:Terser / 自研:白名单/原子写出
↓
⑩ 资源名混淆(可选) —— 自研:arsc-obfuscate
↓
⑪ ZIP 对齐 —— 第三方:zipalign
↓
⑫ APK 签名 —— 第三方:apksigner / 自研:密钥处理/脱敏
↓
⑬ 最终验证 —— 第三方:apksigner verify / 自研:SHA-256
↓
⑭ 生成报告 —— 自研:报告/门禁/SBOM/修复包
```
