...
// DROIDSEAL_BUILD_PLATFORM 允许在非目标平台主机上校验交叉构建产物
const releaseIsWindows = (process.env.DROIDSEAL_BUILD_PLATFORM ?? process.platform) === "win32"
...
const releaseRuntimePackage = releaseIsWindows ? "@opentui/core-win32-x64" : "@opentui/core-linux-x64"
const releaseNativeLibrary = releaseIsWindows ? "opentui.dll" : "libopentui.so"
...
if (!(await exists(buildMetadataPath))) {
  ...
  if (!metadata.embeddedNative?.includes(`${releaseRuntimePackage}/${releaseNativeLibrary}`)) {
    findings.push(`构建元数据未声明嵌入 OpenTUI ${releaseIsWindows ? "Windows x64" : "Linux x64"}原生库`)
  }
  if (
  ...
  if (!executableText.includes(releaseNativeLibrary)) {
    findings.push(`二进制未检测到嵌入的 OpenTUI ${releaseIsWindows ? "DLL" : "原生库"}`)
  }
