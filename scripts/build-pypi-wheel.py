...
平台标签：Windows 构建产出 win_amd64；Linux 构建产出 manylinux_2_17_x86_64
（二进制 glibc 需求 ≤ 2.17，PyPI 不接受裸 linux_x86_64 标签）。
...
def main() -> int:
    # 目标平台(可用 DROIDSEAL_BUILD_PLATFORM 覆盖,便于在非目标平台交叉构建 wheel)
    plat_platform = os.environ.get("DROIDSEAL_BUILD_PLATFORM") or sys.platform
    is_win = plat_platform == "win32"
    plat_name = "win_amd64" if is_win else "manylinux_2_17_x86_64"
    executable_name = "droidseal.exe" if is_win else "droidseal"
    metadata_name = "droidseal-build.json" if is_win else "droidseal-build.linux.json"
    # 只打包当前平台的二进制与元数据(合并 dist 中另一平台的文件不进入 wheel)
    exclude = {"droidseal", "droidseal-build.linux.json"} if is_win else {"droidseal.exe", "droidseal-build.json"}

    if not (dist_dir / executable_name).is_file() or not (dist_dir / metadata_name).is_file():
        print(
            f"[DroidSeal] 缺少 dist/{executable_name} 或 {metadata_name}，请先执行对应平台的 `bun run build`。",
            file=sys.stderr,
        )
        return 1

    # 1. 同步当前平台的二进制产物到 Python 包内(_bin 已被 .gitignore 排除)
    if bin_dir.exists():
    ...
    for old in wheelhouse.glob(f"droidseal-*-{plat_name}.whl"):
        old.unlink()
    ...
    # 3. 校验平台标签(只检查当前平台的 wheel,避免旧平台 wheel 干扰)
    wheel = next(wheelhouse.glob(f"droidseal-*-{plat_name}.whl"), None)
