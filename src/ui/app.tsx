...
export function App() {
  ...
  // OpenTUI's ScrollBoxRenderable.onMouseEvent scrolls on ANY wheel event,
  // ignoring modifiers, and runs even after an ancestor calls stopPropagation.
  // That makes Ctrl+wheel over content both zoom AND drift a few lines. We
  // install an instance-level override that no-ops native scroll for Ctrl
  // wheels; the event still bubbles up (processMouseEvent runs unchanged) so the
  // pane handler performs the zoom. onMouseEvent is protected, so a localized
  // cast is required to patch it at runtime.
  const suppressCtrlWheel = (box: ScrollBoxRenderable) => {
    const patch = box as unknown as { onMouseEvent(event: MouseEvent): void }
    const native = patch.onMouseEvent.bind(box)
    patch.onMouseEvent = (event: MouseEvent) => {
      if (event.type === "scroll" && event.modifiers.ctrl) return
      native(event)
    }
  }
  ...
  const handleInteractionMouseScroll = (event: MouseEvent) => {
    ...
    if (event.modifiers.ctrl) {
      ...
      adjustInteractionZoom(event.scroll.direction === "up" ? "in" : "out")
      return
    }
  ...
  const showHelp = () => {
    addMessage("assistant", "帮助与安全边界", [
      ...
      "左下交互区：Ctrl+加号放大、Ctrl+减号缩小、Ctrl+0 复位，也可以按住 Ctrl 滚动鼠标滚轮。",
  ...
  return (
    <box width="100%" height="100%" flexDirection="column" backgroundColor={theme.background}>
      ...
      >
        ...
        <Show when={dimensions().width >= DROIDSEAL_LOGO_WIDTH + 32}>
          <box flexDirection="column" alignItems="flex-end" flexGrow={1} flexShrink={1}>
            ...
            <text fg={busy() ? theme.accent : theme.success}>{busy() ? "● processing" : "● ready"}</text>
          </box>
        </Show>
      </box>
      ...
      <box flexDirection="row" flexGrow={1} minHeight={0}>
        ...
        <Show when={showSidebar()}>
          ...
          >
            ...
            <box height={1} />
            <scrollbox ref={(el: ScrollBoxRenderable) => { progressScrollBox = el; suppressCtrlWheel(el) }} flexGrow={1} minHeight={0} stickyScroll stickyStart="bottom">
              <For each={steps()}>
                {(step, index) => (
      ...
      >
        ...
        <box flexGrow={1} />
        <text fg={theme.textMuted}>交互区 {interactionZoom()}% · Ctrl+滚轮 / Ctrl± · secrets redacted</text>
      </box>
    </box>
  )
}
