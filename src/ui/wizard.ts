...
export function questionsFor(draft: WizardDraft): WizardQuestion[] {
  ...
  questions.push(
    {
      ...
      detail: "默认地址会显示并预填在输入框中，可直接确认或修改。目录中会生成最终 APK 和隐藏的 .droidseal/runs 运行记录；原始 APK 不会被覆盖。",
  ...
  questions.push(
    ...
    },
    {
      ...
      title: "签名方式",
      prompt: "选择发布签名方式",
      detail: "签名密码只保存在本次进程内存，不写入配置、命令行参数或报告。无密钥可选“生成新密钥”；已有发布密钥可选“使用现有密钥”（并可按需换新）；无需改动签名可“不签名”。",
      choices: [
        { value: "existing", label: "使用现有密钥（已有发布密钥）", detail: "复用已有 JKS/PKCS12 重新签名；更新已上架应用必须用原发布密钥。可选“换新密钥”生成替换密钥库。", shortcut: "1" },
        { value: "create", label: "生成新密钥（尚无发布密钥）", detail: "为尚未发布的新应用创建签名库：填写证书信息后由 DroidSeal 调用 keytool 生成。生成后务必立即备份——密钥一旦丢失将无法再为同一应用发布更新。", shortcut: "2" },
        { value: "skip", label: "不签名", detail: "保留并验证 APK 现有签名；若 APK 没有有效签名则无法正式发布。", shortcut: "3" },
      ],
    },
  )

  if (draft.signingMode === "existing") {
    questions.push({
      id: "renewKey",
      kind: "choice",
      title: "复用还是换新密钥",
      prompt: "是复用现有密钥重新签名，还是替换为全新的密钥？",
      detail:
        "复用：使用原发布密钥对 APK 重新签名（版本更新推荐，可覆盖安装旧版本）。换新：在该路径生成全新的签名库并覆盖原文件。⚠ 新密钥与旧密钥不同，用它签名的 APK 无法覆盖更新用旧密钥签名的应用安装——仅在确实需要更换发布主体或旧密钥已泄露时使用。",
      choices: [
        { value: "no", label: "复用现有密钥", detail: "用原发布密钥重新签名（版本更新推荐）", shortcut: "1" },
        { value: "yes", label: "换新密钥（覆盖生成）", detail: "生成全新密钥库；新密钥无法更新旧安装", shortcut: "2" },
      ],
    })
  }
...
export function applyAnswer(
  ...
  switch (question.id) {
    ...
    case "enableArscObfuscation":
      ...
      break
    case "signingMode":
      if (value !== "existing" && value !== "create" && value !== "skip") throw new Error("请选择有效的签名方式")
      next.signingMode = value
      // “换新密钥”仅在 existing 模式下有意义；切换走时清空，避免污染后续配置。
      next.renewKey = value === "existing" ? Boolean(next.renewKey) : false
      break
    case "renewKey":
      if (value !== "yes" && value !== "no") throw new Error("请选择复用还是换新密钥")
      next.renewKey = value === "yes"
      break
    case "keystorePath":
